...
HALLOWEEN=0	#0 => disabled. 1 => halloween mode
...
SNAP_REGION=1200x800+260+220  #extract picture 1400x800px, starting from offset 520x+0y
...
MEDIA_REGION=1200x600+450+0  #extract picture 1400x800px, starting from offset 520x+0y
#MEDIA_REGION=500x200+228+40  #extract picture 500x200px, starting from offset 228x+40y
MEDIA_SCALE=800 #width of picture that we should want to get on Telegram
...
LIGHT2_IDX=0		# lights disabled: do not turn lights ON before getting snapshot, during the night
...
LIGHT2_SCENEIDX=3	# idx of the scene that activates light2 for 300s or os. Set to 0 to disable
			# Scene must be created by hand: add a scene, assign a name, add the light device and set Off Delay to 300 or other value
DELAY_BEFORE_SNAPSHOT=5 #Delay from turning lights ON and get a snapshot, to permit CCD stabilization. Possible values: 0 (disabled), 0.5 (half second), 1.5 (1.5s), 2 (2s),...			

if [ $HALLOWEEN -ne 0 ]; then 
	LIGHT1_IDX=0
	LIGHT2_IDX=0
fi
...
if [ -n "$SNAPURL" ]; then
	...
	if [ -n "$SNAP_REGION" ]; then
		#extract image region
		convert -extract ${SNAP_REGION} -scale ${SNAP_SCALE} /tmp/snapshot.jpg /tmp/snap.jpg
	else
		mv /tmp/snapshot.jpg /tmp/snap.jpg
	fi
elif [ -n "$MEDIAURL" ]; then
	...
	#extract image region
	convert -extract ${MEDIA_REGION} -scale ${MEDIA_SCALE} /tmp/snapshot.jpg /tmp/snap.jpg
else
	# no SNAPURL, no MEDIAURL => return
	exit 0
fi
